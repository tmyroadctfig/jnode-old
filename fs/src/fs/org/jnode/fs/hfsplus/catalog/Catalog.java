...
public class Catalog {
	 ...
	 * 
	 * @param fs
	 *            HFS+ file system that contains catalog informations.
	 * @throws IOException
	 */
	public Catalog(final HfsPlusFileSystem fs) {
		this.fs = fs;
	}

	/**
	 * Read catalog from the file system.
	 * 
	 * @throws IOException
	 */
	public void read() throws FileSystemException {
		...
		try {
			if (!catalogFile.getExtent(0).isEmpty()) {
				buffer = ByteBuffer
						.allocate(NodeDescriptor.BT_NODE_DESCRIPTOR_LENGTH
	 ...
	 * 
	 * @param params
	 *            The new HFS file system parameters
	 */
	public void create(HFSPlusParams params) {
		...
		//
		int totalNodes = params.getCatalogClumpSize()
				/ params.getCatalogNodeSize();
		int freeNodes = totalNodes - 2;
		log.debug("Create catalog header record.");
		bthr = new BTHeaderRecord(1, 1, params.getInitializeNumRecords(), 1, 1,
				nodeSize, CatalogKey.MAXIMUM_KEY_LENGTH, totalNodes, freeNodes,
	...
	public void update() throws IOException {
		...
		long offset = vh.getCatalogFile().getExtent(0)
				.getStartOffset(vh.getBlockSize());
	...
	private CatalogLeafNode createRootNode(HFSPlusParams params) {
		...
		CatalogFolder folder = new CatalogFolder(params.isJournaled() ? 2 : 0,
		...
		CatalogThread ct = new CatalogThread(
	 ...
	 *            Parent node id
	 * @param nodeNumber
	 *            Index of node where the search begin.
	 * @return Array of LeafRecord
	 * @throws IOException
	 */
	public final LeafRecord[] getRecords(final CatalogNodeId parentID,
			final int nodeNumber) throws IOException {
		try {
